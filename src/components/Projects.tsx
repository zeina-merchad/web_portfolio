...
const ProjectCard: React.FC<{ project: Project; featured?: boolean }> = ({ project, featured }) => (
  <div className={`group border border-ink-200 bg-white p-6 flex flex-col gap-4 hover:border-ink-400 transition-colors ${featured ? 'md:col-span-2' : ''}`}>
      ...
      <div className="flex items-center gap-3 shrink-0 mt-1">
        {project.github && (
          <a href={project.github} target="_blank" rel="noopener noreferrer"
            className="text-ink-400 hover:text-ink-900 transition-colors text-sm" aria-label="GitHub">
        ...
        {project.link && (
          <a href={project.link} target="_blank" rel="noopener noreferrer"
            className="text-ink-400 hover:text-ink-900 transition-colors" aria-label="Live site">
    ...
    <p className="text-ink-600 text-sm leading-relaxed flex-1">{project.description}</p>
      ...
      <div className="px-6 max-w-5xl mx-auto">
        <div className="flex items-end justify-between mb-12">
          <div>
            ...
            <h2 className="font-display text-3xl font-bold text-ink-900">Things I've built</h2>
          </div>
          <span className="font-mono text-xs text-ink-400">{projects.length} total</span>
        </div>
